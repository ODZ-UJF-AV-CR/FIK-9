...
\author[\textit{et~al}]{Jakub Kákona\footnote{Corresponding autor: kakonjak@fel.cvut.cz},$^{ a}$,  Martina Lužová \,$^{ b}$, Martin Kákona \,$^{ b}$, Marek Sommer \,$^{ b}$, Martin Povišer \,$^{ b}$, Ondřej Ploc \,$^{ b}$, Roman Dvořák \,$^{ c}$, Iva Ambrožová\,$^{ b}$}
...
{\begin{tabular}{p{0.15\textwidth} >{\centering\arraybackslash}p{0.15\textwidth} >{\centering\arraybackslash}p{0.15\textwidth} >{\centering\arraybackslash}p{0.15\textwidth}  >{\centering\arraybackslash}p{0.15\textwidth}}
Hypothesis & Empirical value of statistical criterion $t_{exp}$ & Statistical significance level $\alpha$& Critical value of statistical criterion $t_{n_1 + n_2 -2} (\alpha / 2)$ & Decision \\
